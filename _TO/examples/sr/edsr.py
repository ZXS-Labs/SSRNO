import argparse
import torch
from super_image import EdsrModel, ImageLoader, EdsrConfig
...
augmented_dataset = load_dataset(
    "/home/hanlei/Code/_TorchIntegral/data/Div2k.py", f"bicubic_x{args.scale}", split="train"
...
eval_dataset = EvalDataset(
    load_dataset("/home/hanlei/Code/_TorchIntegral/data/Div2k.py", f"bicubic_x{args.scale}", split="validation")
)

# MODEL
# import json 
# tg = json.load(open("/home/hanlei/Code/_TorchIntegral/pretrained/config.json"))
# cfg = EdsrConfig(**tg)
# model = EdsrModel(cfg)
model = EdsrModel.from_pretrained("/home/hanlei/Code/_TorchIntegral/pretrained", scale=args.scale)
...
state_dict = {
    ...
    for key, value in torch.load("/home/hanlei/Code/_TorchIntegral/pretrained/pytorch_model_4x.pt").items()
}
...
if args.integral:
    ...
    for i, group in enumerate(model.groups[:-1]):
        ...
        group.reset_distribution(inn.UniformDistribution(size, 256,1))
        if args.resample:
            group.resize(size)
